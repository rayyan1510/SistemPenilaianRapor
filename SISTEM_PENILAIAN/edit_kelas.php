...
<body class="antialiased min-h-screen relative lg:flex" x-data="{open : false}">
  ...
  <div class="relative z-0 lg:flex-grow">
    ...
    $query = mysqli_query($koneksi, "SELECT * FROM kelases WHERE id_kelas = '$id_kelas'");
    ...
    <div class="flex justify-center flex-wrap py-6 px-6">
      <div class=" w-96 w-12/12 sm:w-12/12 p-3 shadow-sm bg-gray-100 rounded">
        <form action="proses_update_kelas.php" method="POST">
          ...
          <div class="px-12 ">
            <label for="nim" class="block mb-2 text-md text-indigo-500">Nama Kelas</label>
            <input type="text" class="shadow-sm bg-gray-50 border border-indigo-800 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 text-indigo-800 w-64" name="nama_kelas" id="kelas" value="<?= $data['nama_kelas']; ?>" required>
          </div>
          ...
          <div class="flex justify-between items-baseline px-12">
            ...
            <a href="kelas_table.php" class="text-sm hover:underline">Kembali</a>
          </div>
