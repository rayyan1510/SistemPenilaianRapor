...
<body class="antialiased min-h-screen relative lg:flex" x-data="{open : false}">
  ...
  <div class="relative z-0 lg:flex-grow">
    ...
    $query = mysqli_query($koneksi, "SELECT * FROM tahun_ajarans WHERE id_tahun = '$id_tahun'");
    ...
    <div class="flex justify-center flex-wrap py-6 px-6">
      <div class=" w-96 w-12/12 sm:w-12/12 p-3 shadow-sm bg-gray-100 rounded">
        <form action="proses_update_tahun.php" method="POST">

          <input type="hidden" name="id_tahun" value="<?= $data['id_tahun']; ?>">

          <div class="px-12 ">
            <label for="tahun_ajaran" class="block mb-2 text-md text-indigo-500">Tahun Ajaran</label>
            <input type="text" class="shadow-sm bg-gray-50 border border-indigo-800 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 text-indigo-800 w-64" name="tahun_ajaran" id="tahun_ajaran" value="<?= $data['tahun_ajaran']; ?>" required>
          </div>
          ...
          <div class="flex justify-between items-baseline px-12">
            ...
            <a href="tahun_table.php" class="text-sm hover:underline">Kembali</a>
          </div>
