...
<body class="antialiased min-h-screen relative lg:flex" x-data="{open : false}">
  ...
  <div class="relative z-0 lg:flex-grow">
    ...
    $query = mysqli_query($koneksi, "SELECT * FROM dosens WHERE id_dosen = '$id_dosen'");
    ...
    <div class="flex justify-center flex-wrap py-6 px-6">
      <div class=" w-96 w-12/12 sm:w-12/12 p-3 shadow-sm bg-gray-100 rounded">
        <form action="proses_update_dosen.php" method="POST">
          ...
          <div class="px-12 ">
            <label for="nim" class="block mb-2 text-md text-indigo-500">Nip</label>
            <input type="text" class="shadow-sm bg-gray-50 border border-indigo-800 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 text-indigo-800 w-64" name="nip" value="<?= $data['nip']; ?>" id="nip" required>
          </div>

          <div class="px-12 ">
            <label for="nama" class="block mb-2 text-md text-indigo-500">Nama Dosen</label>
            <input type="text" class="shadow-sm bg-gray-50 border border-indigo-800 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 text-indigo-800 w-64" name="nama" id="nama" value="<?= $data['nama_dosen']; ?>" required>
          </div>
          ...
          <div class="flex justify-between items-baseline px-12">
            ...
            <a href="dosen_table.php" class="text-sm hover:underline">Kembali</a>
          </div>
